...
EigenPrism <- function(y,X,
   ...
   CI = her.trunc(est/yvar) + sqrt(v)*qnorm(1-alpha/2)*c(-1,1)

   # Generate list with results
   result=list()
   result$estimate = her.trunc(est/yvar)
   result$CI = CI
   #result$para.est = w*(t(svd$u)%*%y)^2
   return(result)
}
EigenPrism<-compiler::cmpfun(EigenPrism)
...
scaled.lasso <- function(X, y = NULL, lam0 = NULL) {
  ...
  list(sigma2=as.numeric(sigmanew), non.zeros = sum(hbeta!=0)  )
}
scaled.lasso <- compiler::cmpfun(scaled.lasso)
...
mm2dicker = function(y,x,CI = FALSE){
  ...
  sum.Xty2 =  sum(crossprod(x,y)^2)
  tau2.dicker = sum.Xty2*m1/( n*(n+1)*m2 ) -  mean(y^2)*p*m1^2 / ((n+1)*m2^2)
  ...
  confident = her.trunc(tau2.dicker/ mean(y^2) ) - c(-1,1)*log(.5)*sqrt(p)/n#+ phi0*qnorm(1-0.05/2)*c(-1,1)/sqrt(n)
  list(est = her.trunc(tau2.dicker/ mean(y^2) ),CI = confident)
  }
}
mm2dicker <- compiler::cmpfun(mm2dicker)

ridge.mle <- function(yTilde,Lambda,eps){
  max.iter <- 500
  cat('EM MLE-RIDGE  ')
  sig2em <- 2
  tau2em <- 2
  ...
  for(i in 1:max.iter){
    sig2emN <- p*sum(yTilde^2/(p+tau2em/sig2em*Lambda*p)^2)/sum(1/(p+tau2em/sig2em*Lambda*p))
    tau2emN <- (p/n)*sum(yTilde^2/(sig2em/tau2em*p+Lambda*p))
    
    if (abs(sig2emN-sig2em) < eps){
      sig2em <- sig2emN
      tau2em <- tau2emN  
      break;
    }
  ...
  if(i == max.iter) cat('PROBLEM!\n')
  cat('done!\n')
  list(sig2hat = sig2emN,tau2hat = tau2emN)
}
ridge.mle<-compiler::cmpfun(ridge.mle) 